...
<details>
<summary>Exercise 1 — RFC Autopsy</summary>

**How to structure a retroactive RFC:**

The retroactive RFC's value is not documentation — it is gap analysis. Every section you cannot complete reveals what was unknown at decision time.

**Seven-section checklist:**

| Section | What it should contain | Common failure mode |
|---------|----------------------|---------------------|
| Context | What system state existed; constraints; who was affected | Starts with "We decided to…" — skips the situation |
| Problem Statement | Precise gap with evidence (metrics, user reports, incidents) | Lists symptoms instead of the root problem |
| Proposal | The decision that was made, with enough specificity to understand the choice | Too high-level ("we switched to gRPC") — no details about config, phases, or scope |
| Alternatives | At least 2 options a reasonable engineer would have genuinely considered | Strawman alternatives ("we could do nothing") that no one would actually choose |
| Trade-offs | What the chosen solution gives up | Missing — most retrospective RFCs have no trade-offs section because the author is defending the past decision |
| Decision Criteria | How you will know in 90 days this was right — specific, measurable | Vague: "better performance" with no threshold |
| Open Questions | What remains unresolved after the decision | Missing — questions that were never tracked |

**The strawman test for alternatives:** "Would an experienced engineer working on this problem at this time have seriously proposed this option?" If no, it is a strawman — remove it and replace with a real option.

**Staff-level phrasing:** "A retroactive RFC is a reckoning, not a defense — every section you can't write reveals a decision that was made on insufficient information, and writing the RFC is how you learn to do it better next time."
</details>

<details>
<summary>Exercise 2 — Alternatives Audit</summary>

**Rubric for evaluating an alternatives section:**

**(a) Real vs strawman alternatives:**
Real alternatives have a plausible champion — someone on the team would actually prefer this option under some reasonable set of priorities. Apply the test: "If I described Option B to an experienced engineer working on this problem, would they say 'yes, that's a legitimate approach'?"

Strawman signals: the alternative is the trivial "do nothing," an option that violates a hard constraint stated in the Problem Statement, or an option that requires technology the team has zero expertise in and which is offered with no mitigation.

**(b) Specific rejection reasons:**
| Vague (fails) | Specific (passes) |
|--------------|------------------|
| "Too complex" | "Requires maintaining a custom synchronization mechanism; the only team member with expertise left the company in Q3" |
| "Worse performance" | "Adds 150ms serialization overhead per call based on our benchmark in the Code Lab — 5× our 30ms budget" |
| "Not scalable" | "Single-node ZooKeeper coordinator becomes the bottleneck above 50k ops/sec; our current peak is 43k/s and growing 8% monthly" |

**(c) Finding a missing preferred option:**
This is the most important question. An RFC written by someone who made the decision before writing the RFC will reverse-engineer alternatives to justify the conclusion already reached. The tell: every alternative has a decisive flaw that happens to exactly align with the chosen option's strengths. If you find this pattern, the alternatives section needs a rewrite, not editorial polish.

**Good public example to use:** Apache Kafka KIP-500 (ZooKeeper removal). Its alternatives section genuinely evaluates "improve ZooKeeper integration" as a real option with real tradeoffs before explaining why the self-managed metadata quorum was chosen.
</details>

<details>
<summary>Exercise 3 — Sticky RFC Design</summary>

Three process changes that make the RFC the decision artifact:

**Change 1: RFC-gated deployments**
Mechanism: The CI/CD pipeline checks that the deployment ticket references a merged RFC for changes that touch more than 2 services, schema migrations, or new external dependencies. Implementation: a GitHub Actions step that checks the ticket description for an RFC link before approving the deployment workflow. Without the RFC link, the deployment gate is red.

Why this works: It moves the RFC from "optional documentation" to a literal deploy prerequisite. Engineers who skipped writing the RFC now feel the consequence at deploy time, not in retrospect.

**Change 2: Named reviewers with deadline and state machine**
Mechanism: Every RFC must list 2 named reviewers who commit to voting (Approve / Request Changes) within 5 business days. The RFC cannot transition from "In Review" to "Decided" without both votes. Reviewers are auto-assigned via a rotation tool; they can delegate but must name a substitute.

Why this works: Diffuse responsibility ("anyone can review") means no one does. Named responsibility with a deadline creates accountability. The 5-day window prevents indefinite delay while not creating urgent-deadline panic.

**Change 3: Weekly RFC slot in team sync**
Mechanism: The last 10 minutes of the weekly team sync are reserved for RFC status review. Any RFC in "In Review" state for more than 5 business days without a vote gets flagged. Any decision that happened in Slack/Jira/meeting gets redirected to the RFC process: "That's an RFC-level decision — can you open one by Thursday?"

Why this works: Social accountability. Engineers know decisions made outside the RFC process will be called out publicly. The repeat pattern of "Slack decision" → RFC redirect eventually changes the default channel for decisions.

**Staff-level phrasing:** "An RFC process fails when the RFC is a post-hoc artifact — the decision was made, then the RFC was written to justify it. The fix is making the RFC a prerequisite for the decision, not a record of it."
</details>

<details>
<summary>Exercise 4 — Open Questions Severity</summary>

**For each open question in a gRPC RFC, the unfavorable scenario analysis:**

**Open Question: "Does Envoy 1.24 upgrade fit within Q2 roadmap?"**

If unfavorable (upgrade slips to Q3): Phase 2 (gRPC migration) cannot start until Envoy 1.24 is deployed — without the proxy, gRPC connections cannot be load-balanced to external clients. Options if this happens:
- (a) Delay the full migration to Q3 and proceed with Phase 1 (internal service-to-service) only using grpc-java direct connections
- (b) Use a temporary Envoy 1.23 workaround with custom gRPC plugin — adds 3 weeks of infrastructure work, high operational risk
- (c) Replace Envoy with HAProxy 2.6 which supports gRPC natively — different operational model, not on the team's roadmap

Pre-writing this paragraph in the RFC forces the author to think through the contingency before it becomes an incident.

**Template for writing severity analysis:**
For each open question:
1. State the favorable outcome: "Envoy upgrade completes in Q2 as planned"
2. State the unfavorable outcome: "Envoy upgrade slips to Q3"
3. Identify what changes: Does the entire proposal slip? Is there an alternative path?
4. Identify who decides: If Q3 slips, who approves the contingency plan?

An open question without this analysis is a question without a decision tree — it records uncertainty without enabling action.
</details>

<details>
<summary>Exercise 5 — Decision Criteria Falsifiability</summary>

**Falsifiability test:** "Could an engineer, 6 months from now, open a specific dashboard and definitively answer 'met' or 'not met' for this criterion?"

**Applying to common RFC decision criteria:**

| Original (fails test) | Rewritten (passes test) |
|----------------------|------------------------|
| "The system should handle increased load gracefully" | "p99 latency under 2× peak load (8,000 req/s) stays below 100ms, verified by a load test in staging within 30 days of deployment" |
| "Developers should be able to integrate easily" | "Time-to-first-successful-API-call for a new team drops from 4 hours (current measured avg) to under 45 minutes, validated via 3 new-team integration sessions" |
| "The solution should be maintainable" | "On-call alert volume for this component is below 2 pages/week at 90 days post-deploy, compared to 7 pages/week baseline" |
| "gRPC migration should improve performance" | "Internal service-to-service p99 latency drops by ≥30% within 90 days of Phase 1 completion, measured on the order-payment service pair" |

**Pattern for writing falsifiable criteria:**
`[metric] [direction] [threshold] [within timeframe] [measured at location/method]`

The threshold and measurement method must be defined at RFC-writing time, not retroactively. A criterion that is only measurable if you define the threshold after seeing the result is not a criterion — it is a post-hoc justification.

**Staff-level phrasing:** "A decision criterion is falsifiable if and only if it can be proven false. 'Handles load gracefully' cannot be proven false — someone will always claim the load was handled gracefully enough. '100ms p99 at 2× peak' can be proven false by a single load test result."
</details>
