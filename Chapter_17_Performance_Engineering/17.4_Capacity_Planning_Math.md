...
<details>
<summary>Exercise 1 — Little's Law Application</summary>

**Base case: 50ms processing time**
Little's Law: L = λW
- λ = 5,000 messages/second
- W = 0.050 seconds (50ms)
- L = 5,000 × 0.050 = **250 concurrent threads**

**Spike case: 500ms processing time**
- λ = 5,000 messages/second (unchanged — messages still arriving)
- W = 0.500 seconds (500ms)
- L = 5,000 × 0.500 = **2,500 concurrent threads**

**Operational implication for circuit breaker design:**

The 10x latency spike (50ms → 500ms) requires 10x more threads — from 250 to 2,500. Without intervention, the current 250-thread pool exhausts in ~0.5 seconds. When threads are exhausted, new messages queue up, consuming memory, until the consumer falls further behind and the Kafka consumer group lags grow unbounded.

The circuit breaker must trip **before thread exhaustion**, not after. Design trigger: when downstream processing time exceeds 200ms (4× baseline), open the circuit immediately. At 200ms, required threads = 5,000 × 0.200 = 1,000 — already 4× the pool size. Waiting for 500ms (when exhaustion is complete) gives you nothing to protect.

Concrete configuration:
```java
CircuitBreakerConfig.custom()
    .slowCallDurationThreshold(Duration.ofMillis(200))  // 4× baseline
    .slowCallRateThreshold(50.0f)                       // trip if 50% of calls are slow
    .waitDurationInOpenState(Duration.ofSeconds(30))
    .build()
```

**Staff-level phrasing:** "Little's Law makes the circuit breaker threshold calculation concrete: a 4× latency spike with a fixed thread pool means 4× thread exhaustion rate — the circuit should trip at the first sign of slowness, not after the pool is already empty."
</details>

<details>
<summary>Exercise 2 — Amdahl's Law Calculation</summary>

**Given:** Total runtime = 60 min. Parallelizable = 30 min. Serial = 30 min.

**(a) Parallel fraction p:**
p = 30/60 = **0.5 (50%)**

**(b) Maximum speedup with infinite cores:**
Amdahl's Law: S(∞) = 1 / (1 − p) = 1 / (1 − 0.5) = 1 / 0.5 = **2×**

At infinite cores, parallelizable part takes 0ms. Total time = 30 min serial + 0 min parallel = 30 min.
60 min / 30 min = 2×.

**(c) Actual speedup with 4 cores:**
S(4) = 1 / ((1 − p) + p/N) = 1 / (0.5 + 0.5/4) = 1 / (0.5 + 0.125) = 1 / 0.625 = **1.6×**
New runtime: 60 / 1.6 = **37.5 minutes**

**(d) "Will 16 cores cut it in half?" — 3× speedup needed for 60→20 min:**
S(16) = 1 / (0.5 + 0.5/16) = 1 / (0.5 + 0.03125) = 1 / 0.53125 ≈ **1.88×**
New runtime: 60 / 1.88 ≈ **32 minutes**

**Answer: No.** 16 cores achieves 1.88×, giving 32 minutes — not 20 minutes. The serial floor of 30 minutes is absolute. To cut total time in half (to 30 min) requires infinite cores. To reach 20 minutes is mathematically impossible with 50% serial code, regardless of hardware.

**The business message:** "Before buying 16 cores, measure the serial fraction. A 50% serial job has a theoretical maximum of 2× speedup from all the hardware in the world — investigate reducing the serial portion (locks, sequential I/O, single-threaded coordination) before scaling horizontally."
</details>

<details>
<summary>Exercise 3 — M/M/1 Sizing</summary>

**Given:** N = 20 connections, service time W = 25ms = 0.025s

**M/M/1 model:** N workers, arrival rate λ, utilization ρ = λW/N

**At 80% utilization (ρ = 0.80):**
ρ = λW/N → λ = ρN/W = 0.80 × 20 / 0.025 = **640 req/s**

At 640 req/s the pool runs at 80% utilization. This is near the safe operating limit.

**At what λ does mean latency exceed 100ms?**

M/M/1 mean wait time (including service): T = W / (1 − ρ)

Set T = 100ms = 0.100s:
0.100 = 0.025 / (1 − ρ)
1 − ρ = 0.025 / 0.100 = 0.25
ρ = **0.75**

λ at ρ = 0.75:
λ = 0.75 × 20 / 0.025 = **600 req/s**

**Summary table:**

| λ (req/s) | Utilization ρ | Mean latency T | Notes |
|-----------|--------------|----------------|-------|
| 400 | 50% | 0.025/(1-0.5) = 50ms | Comfortable |
| 600 | 75% | 0.025/(1-0.75) = 100ms | **SLA boundary** |
| 640 | 80% | 0.025/(1-0.80) = 125ms | SLA violated |
| 760 | 95% | 0.025/(1-0.95) = 500ms | Latency explosion |

**Conclusion:** The SLA of 100ms breaks at 600 req/s (75% pool utilization) — well below the 80% "safe" limit heuristic. Size the pool for 70% peak utilization to maintain 85ms headroom: target pool = λ × W / 0.70 = 640 × 0.025 / 0.70 = **23 connections** (round up to 25 for headroom).
</details>

<details>
<summary>Exercise 4 — ThreadPoolFactory</summary>
...
public class ThreadPoolFactory {
    ...
    public ThreadPoolFactory(double lambdaRps, double avgServiceTimeSec) {
        int littlesLaw = (int) Math.ceil(lambdaRps * avgServiceTimeSec);
        this.poolSize = (int) Math.ceil(littlesLaw * 1.3);  // 30% headroom

        this.executor = new ThreadPoolExecutor(
            poolSize,
            poolSize,
            60L, TimeUnit.SECONDS,
            new LinkedBlockingQueue<>(poolSize * 2),        // bounded queue
            new ThreadFactory() {
            ...
            new ThreadPoolExecutor.CallerRunsPolicy()       // backpressure: caller executes when full
        );
    }
    ...
    public void printHealthCheck() {
        ...
        double utilization = (double) active / poolSize;

        if (utilization > 0.70) {
            System.out.printf("[WARN] Thread pool utilization=%.0f%% (active=%d/%d)%n",
                utilization * 100, active, poolSize);
        }
        if (queued > 0) {
            System.out.printf("[WARN] Thread pool queue non-empty: %d tasks waiting%n", queued);
        }
    }
}
```
...
**Usage example:**
```java
// λ = 1000 req/s, avg latency W = 0.05s → Little's Law: L = 50, +30% = 65 threads
ThreadPoolFactory factory = new ThreadPoolFactory(1000.0, 0.05);
ThreadPoolExecutor pool = factory.getExecutor();

// Schedule health check every 10 seconds
ScheduledExecutorService monitor = Executors.newSingleThreadScheduledExecutor();
monitor.scheduleAtFixedRate(factory::printHealthCheck, 10, 10, TimeUnit.SECONDS);
```

**Why `CallerRunsPolicy`:** When the queue fills (pool exhausted + 2×poolSize queued tasks), the calling thread executes the task itself. This creates natural backpressure — the upstream producer slows down because it's now doing work. Alternative: `AbortPolicy` fails fast with `RejectedExecutionException` and lets the caller decide to retry/drop/circuit-break.

**Common mistake:** Using an unbounded `LinkedBlockingQueue` (no capacity argument) — the queue grows without bound when the pool is saturated, consuming heap until OOM. Always bound the queue.
</details>

<details>
<summary>Exercise 5 — URL Shortener Back-of-Envelope</summary>

**Given:** 100M DAU, 1 creation/user/day, 5 clicks/user/day, 86,400 seconds/day

**(a) RPS calculations:**

Write RPS (URL creation):
```
100M users × 1 create/day ÷ 86,400 s/day = 1,157 writes/sec ≈ 1.2k writes/s
```

Read RPS (URL redirects):
```
100M users × 5 clicks/day ÷ 86,400 s/day = 5,787 reads/sec ≈ 5.8k reads/s
```

Read:Write ratio = 5:1. Read-heavy, write rate is manageable.

**(b) 5-year storage:**

Daily URL creation: 100M URLs/day
5-year total: 100M × 365 × 5 = **182.5 billion URLs**

Storage at 500 bytes/record:
```
182.5B × 500 bytes = 91.25 TB ≈ 91 TB
```

With 3× replication (standard for distributed storage): **~273 TB raw storage**. At $0.02/GB/month on object storage: $273,000 × $0.02 = ~**$5,460/month** for storage alone.

**(c) Network bandwidth for reads:**

5,787 reads/s × 100 bytes/response = 578,700 bytes/s ≈ **565 KB/s ≈ 4.5 Mbps**

This is trivially small — bandwidth is not the constraint. Even at 10× that: 45 Mbps. A single 1Gbps NIC handles this with ease. Latency (redirect round-trip) is the user-facing constraint, not bandwidth.

**(d) Cache sizing for 95% read hit rate:**

URL access follows a power-law distribution (Zipf): a small fraction of URLs receives the vast majority of clicks. Typically:
- Top 1% of URLs = ~50% of clicks
- Top 10% of URLs = ~80-90% of clicks

To serve 95% of reads from cache:
- Active URLs receiving any clicks: assume 20% of created URLs are "active" = 0.20 × 182.5B = 36.5B URLs total over 5 years, but realistically ~30-day active window = 100M × 30 = 3B recently created URLs
- Cache the top 10% of those: 3B × 0.10 = **300M URLs**
- Cache size: 300M × 500 bytes = **150 GB**

This fits in a Redis cluster (5 nodes × 32GB = 160GB). At a 95% hit rate, only 5% of reads hit the database:
```
5,787 reads/s × 0.05 = 289 DB reads/s — well within a single PostgreSQL node's capacity
```
...
| Write RPS | ~1.2k/s |
| Read RPS | ~5.8k/s |
| 5-year storage | ~91 TB (273 TB with 3× replication) |
| Read bandwidth | ~4.5 Mbps |
| Cache to hit 95% reads | ~150 GB (300M URLs × 500 bytes) |
| DB reads at 95% cache hit | ~290 reads/s |

**Staff-level phrasing:** "Back-of-envelope answers three questions: what is the load, what is the storage, and does a single box handle it — the goal is not precision but order-of-magnitude validation that your design is even possible before you open a code editor."
</details>
