...
<details>
<summary>Exercise 1 — B-tree and the leading-wildcard problem</summary>

**Why `LIKE 'red%'` uses the index but `LIKE '%red%'` cannot:**

A B-tree index stores values in lexicographic order. For a prefix query (`LIKE 'red%'`), the engine seeks to the first entry ≥ `"red"` and scans forward until the entry exceeds `"ree"` — a bounded range scan touching only a small slice of the tree.

```
B-tree leaf nodes (sorted):
[apple | banana | red | red-nike | redneck | shoe | zebra]
         ↑ skip  ↑ start here         ↑ stop here
LIKE 'red%' → seek("red"), scan forward until > "ree" → 3 nodes, O(log N + k)
```

For a leading-wildcard query (`LIKE '%red%'`), the target string can appear *anywhere*: `"infrared"`, `"blood-red-coral"`, `"garment-redneck"`. There is no anchor point in the B-tree where the engine can seek — every leaf must be visited.

```
LIKE '%red%' → cannot seek to any position → full scan of all N leaves
```

The same reasoning applies to `LOWER(p.name) LIKE ...` — the function call destroys the sort order the index encodes, even for prefix queries.

**Staff-level phrasing:** "B-tree indexes encode prefix order; a leading wildcard makes every key a potential match, collapsing a bounded seek into an unbounded sequential scan — the correct solution is an inverted index engine like Elasticsearch that maps tokens to document IDs directly."

</details>

<details>
<summary>Exercise 2 — CDC consumer offline: alerting strategy</summary>

**Metric to alert on:** `kafka.consumer.lag` on the `postgres.public.products` consumer group (`es-sync`). This metric counts the number of messages in the topic that the consumer has not yet processed.

**Alert threshold:**
- Warning: lag > 500 events for > 2 minutes (normal burst recovery is fast; sustained lag signals a real outage)
- Critical: consumer group has 0 active members for > 1 minute (consumer is down, lag is growing)

**Alerting rule (Prometheus notation):**
```promql
# Consumer is down
kafka_consumer_group_members{group="es-sync"} == 0

# Lag growing beyond SLA
kafka_consumer_group_lag{group="es-sync", topic="postgres.public.products"} > 500
  for 2m
```

**Customer-visible impact of 10 minutes of stale results:**
During the 10-minute outage, 2,000 product updates are invisible to searchers:
- Price changes: customers see stale prices in search results, then different prices on the product page → cart abandonment
- Stock changes: out-of-stock products still appear as available → add-to-cart errors
- New products: not yet indexed → missed discovery
- Deleted/recalled products: still searchable → potential customer orders for unavailable items

**Acceptable SLA for a product catalog:** 5 minutes of stale data is tolerable (not financial data); 10 minutes is borderline — the alerting system should page on-call at the 2-minute mark so the consumer is restarted before the 5-minute SLA is breached.

**Staff-level phrasing:** "CDC lag is the observable price of CQRS eventual consistency — instrument consumer lag as a first-class SLI with a budget aligned to the catalog's freshness SLA, not as an afterthought."

</details>

<details>
<summary>Exercise 3 — Elasticsearch delete failure: retry + idempotency</summary>

**Resulting inconsistency:** The product is deleted from Postgres (no longer in the catalog) but the Elasticsearch document survives. Users can find the deleted product via search — clicking it returns a 404 from the product detail API or shows "unavailable." This is a **phantom record**: the search engine lies about what exists.
...
public void onProductChange(ProductChangeEvent event) {
    if ("d".equals(event.operation())) {
        deleteWithRetry(String.valueOf(event.before().id()));
    }
}

private void deleteWithRetry(String productId) {
    int attempts = 0;
    while (attempts < 4) {
        try {
            esClient.delete(d -> d.index("products").id(productId));
            return; // success
        } catch (IOException e) {
            attempts++;
            if (attempts == 4) {
                deadLetterPublisher.publish("es-delete-dlq", productId, e);
                return;
            }
            Thread.sleep(100L * (1L << attempts)); // 200ms / 400ms / 800ms backoff
        }
    }
}
```

**Idempotency guarantee:** Elasticsearch `delete()` by document ID is naturally idempotent — deleting an already-absent document returns `result: "not_found"` (not an error when `ignore_unavailable=true`). Calling delete twice is safe.

**Periodic reconciliation (defense in depth):**
```java
@Scheduled(cron = "0 3 * * *") // nightly at 3 AM
public void reconcile() {
    // Scroll all Elasticsearch product IDs
    // Cross-reference against Postgres catalog
    // Delete ES documents whose IDs no longer exist in Postgres
}
```

**Staff-level phrasing:** "ES delete idempotency means retrying is safe with no duplicate-delete guard needed — push failures to a DLQ for human review and run a nightly reconciliation job as a correctness backstop."

</details>

<details>
<summary>Exercise 4 — Coding: ProductIndexSyncConsumer.reindex()</summary>
...
public class ProductIndexSyncConsumer {
    ...
    private final ElasticsearchClient esClient;
    private final ProductRepository productRepo; // Postgres JPA repository

    public ProductIndexSyncConsumer(ElasticsearchClient esClient,
                                    ProductRepository productRepo) {
        this.esClient = esClient;
        this.productRepo = productRepo;
    }

    // Idempotent: calling twice with same productId produces same ES state (upsert)
    public void reindex(long productId) throws IOException {
        ...
        esClient.index(i -> i
                ...
                .document(toSearchDoc(product)) // upsert — creates or overwrites
        );
    }

    // Original CDC listener unchanged
    @KafkaListener(topics = "postgres.public.products", groupId = "es-sync")
    public void onProductChange(ProductChangeEvent event) throws IOException {
        switch (event.operation()) {
            case "c", "u" -> esClient.index(i -> i
                    .index("products").id(String.valueOf(event.after().id()))
                    .document(toSearchDoc(event.after())));
            case "d" -> esClient.delete(d -> d
                    .index("products").id(String.valueOf(event.before().id())));
        }
    }

    private ProductSearchDoc toSearchDoc(Product p) {
        return new ProductSearchDoc(p.getId(), p.getName(), p.getDescription(),
                p.getBrand(), p.getPrice(), p.getStockQuantity(), p.getTags());
    }
}
```

**Test using Testcontainers ElasticsearchContainer:**
```java
@Testcontainers
@SpringBootTest
class ProductIndexSyncConsumerTest {
    ...
    @Autowired ProductIndexSyncConsumer consumer;
    @Autowired ElasticsearchClient esClient;
    @MockBean ProductRepository productRepo;

    @Test
    void reindex_makesProductSearchable() throws IOException {
        Product product = new Product(42L, "Red Nike Air", "Comfortable running shoe",
                "Nike", new BigDecimal("120.00"), 50, List.of("shoes", "running"));
        when(productRepo.findById(42L)).thenReturn(Optional.of(product));

        consumer.reindex(42L);

        // Calling twice should produce same result (idempotency)
        consumer.reindex(42L);

        SearchResponse<ProductSearchDoc> response = esClient.search(s -> s
                .index("products")
                .query(q -> q.match(m -> m.field("name").query("Red Nike"))),
                ProductSearchDoc.class);

        assertThat(response.hits().hits()).hasSize(1);
        assertThat(response.hits().hits().get(0).source().id()).isEqualTo(42L);
        // Verify repository called exactly twice (once per reindex call — no caching)
        verify(productRepo, times(2)).findById(42L);
    }
}
```

**Why this works:** ES `index()` is an upsert by document ID — it overwrites any existing document with the same ID. Calling `reindex()` twice is safe and produces identical ES state. The test verifies both search discoverability and idempotency.

**Common mistake:** Using `esClient.create()` instead of `esClient.index()` — `create()` throws `409 Conflict` if the document already exists, breaking idempotency for disaster recovery use cases.

</details>

<details>
<summary>Exercise 5 — When to add vector search</summary>

**Catalog size trigger:** ~100K+ products with rich descriptions, where keyword-based matching fails to surface semantically relevant results. Below 100K, curated synonym lists and field boosting (`name^3`) provide adequate relevance. Above 100K with diverse product descriptions, users start searching in natural language ("comfortable running shoes") that doesn't match the product's keyword vocabulary ("cushioned athletic footwear").

**Update frequency consideration:** Less critical than catalog size — vector index refresh is async (rebuild embeddings via batch job nightly or on product create/update event); 100–500ms embedding generation per product via ML inference API is acceptable for async pipelines.

**Elasticsearch index mapping change:**
```json
{
  "mappings": {
    "properties": {
...
**ML model options:**
- `sentence-transformers/all-MiniLM-L6-v2` (open source, 384 dims, ~80MB): runs on-premise, no API cost, 14K sentences/sec on CPU
- OpenAI `text-embedding-3-small` (1536 dims): $0.02/1M tokens, no infrastructure, higher quality
- For 500K products: open-source model preferred (one-time cost, no per-product API fee)

**Hybrid search at scale:** combine BM25 keyword score with k-NN vector score using Elasticsearch's `reciprocal_rank_fusion` — keyword handles exact product names, vector handles semantic queries.

**Staff-level phrasing:** "Add vector search when user queries are semantic (natural language intent) rather than lexical (exact keywords) — the break-even is when embedding infrastructure cost is less than the revenue lost to failed search sessions."
