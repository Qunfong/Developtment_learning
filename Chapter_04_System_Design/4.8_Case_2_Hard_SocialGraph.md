...
<details>
<summary>Exercise 1 — Bi-directional friendship data model in Redis</summary>

A bi-directional friendship between user A and user B requires two Redis Set keys: `friends:A` containing B's ID, and `friends:B` containing A's ID. Each direction is stored explicitly because Redis Sets have no concept of symmetric relationships — reading A's friends only touches `friends:A`, so the reverse must be maintained separately. If the bug causes only `SREM friends:A B` to execute without the corresponding `SREM friends:B A`, the data becomes asymmetric: A no longer sees B in their friend list, but B still sees A. This silent inconsistency causes confusing UX (B thinks they are friends with A, but A does not see B) and can pollute friends-of-friends recommendations with ghost relationships that are impossible to clean up without a full graph consistency scan.

**Staff-level phrasing:** "Bi-directional social graph edges must always be written and deleted atomically as a pair — any half-applied mutation leaves the graph in an asymmetric state that is invisible to query logic and can only be detected by an expensive consistency audit."

</details>

<details>
<summary>Exercise 2 — Meta Tao two-tier cache: reads from followers, writes to leaders</summary>

In Tao's topology, each geographic region has one leader and multiple follower replicas. Writes always route to the leader because the leader is the single authoritative source of record for invalidations — it broadcasts cache invalidation messages to all followers after a write. Reads are served from followers because followers vastly outnumber leaders and serve the same hot friend lists repeatedly without coordination overhead. The consistency anomaly this creates is a read-your-writes violation: a user who adds a friend on the leader may immediately read from a follower that has not yet received the invalidation, seeing the old friend list for a brief window (typically milliseconds to seconds). This is accepted in social graphs because the domain contract is "friends appear eventually" — users tolerate a brief delay in seeing a new friendship, and the alternative (strong read consistency requiring leader reads for every query) would collapse under the 1 billion reads/second throughput requirement.

**Staff-level phrasing:** "Tao deliberately trades read-your-writes consistency for horizontal read scalability — routing writes to leaders and reads to followers means a user may not immediately see their own friendship addition, a trade-off that is only acceptable because social graph staleness is a UX nuisance, not a safety or financial invariant."

</details>

<details>
<summary>Exercise 3 — Write-through vs write-back for social graph edges</summary>

Write-through caching updates both the cache and the database synchronously on every write, guaranteeing that the database always reflects the current state of the cache. Write-back (write-behind) caching writes to the cache immediately and flushes to the database asynchronously, typically via a queue or background task. Write-back can cause data loss if the cache node crashes between the write and the flush — any friendship edges buffered in memory but not yet persisted are gone. This risk is accepted in social graphs because the failure mode (a user's friend list reverting to its pre-crash state) is a minor UX regression, not a financial or safety issue. The same trade-off would be catastrophic for a bank balance, where write-back would allow a node crash to silently erase credits or debits, producing an incorrect account balance that is legally and financially unacceptable.

**Staff-level phrasing:** "Write-back caching trades durability for write throughput — it is valid only in domains where the data can be reconstructed or the loss is tolerable, which social graph edges satisfy and financial balances fundamentally do not."

</details>

<details>
<summary>Exercise 5 — Redis adjacency list sharding strategy at 500M users</summary>

A single Redis instance begins to require sharding when the total adjacency list data exceeds the available RAM of a single node, or when write throughput (new friendships, unfriending) saturates a single Redis thread. At 500M users with an average of 300 friends each, stored as int64 IDs in a listpack-encoded Set, the total footprint is approximately 500M × 300 × 8 bytes = 1.2 TB — well beyond a single machine. The natural partition key is the user ID: `friends:{userId}` hashes to a shard using consistent hashing (Redis Cluster's 16,384-slot CRC16 model). The cross-shard challenge for `SUNIONSTORE` is that each direct friend's key may live on a different shard, so a friends-of-friends computation cannot be done with a single server-side Redis command. The solution is to fetch each friend list from its respective shard in parallel (scatter), union the results in the application layer (gather), then filter and rank — accepting that friends-of-friends is no longer a pure server-side Redis operation but a coordinated scatter-gather across the cluster with O(fan-out) network calls.

**Staff-level phrasing:** "Sharding a Redis social graph by user ID preserves O(1) direct friend lookups per shard but converts friends-of-friends into an application-layer scatter-gather across shards — the key design decision is whether to accept this fan-out cost or pre-materialise second-hop relationships into denormalized adjacency lists."
