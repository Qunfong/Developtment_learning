...
<details>
<summary>Exercise 1 — Vector clock comparison: concurrent or ordered events</summary>

London: `[L:1, N:0]` and NYC: `[L:0, N:1]` are **concurrent** events.

**Comparison rule:** Event A happens-before B if A's clock ≤ B's in every dimension and strictly less in at least one. Concurrent = neither dominates the other.

`[L:1, N:0]` vs `[L:0, N:1]`: L-dimension: London=1 > NYC-event=0, so London doesn't happen-before NYC. N-dimension: London=0 < NYC=1, so NYC doesn't happen-before London. **Concurrent**.

**What the ledger should do:** Apply a deterministic tie-breaker — e.g., higher-amount transaction first, then UUID as secondary key. Both transactions must be applied (neither discarded); the tie-breaker only determines order of application. For a deposit+withdrawal pair, check balance after both to ensure invariants hold.

**Staff-level phrasing:** "Clocks `[1,0]` and `[0,1]` are concurrent — neither happens-before the other; financial concurrent transaction conflict requires a deterministic tie-breaker (higher-amount-first + UUID) applied identically on every node to prevent divergent state."

</details>

<details>
<summary>Exercise 2 — Google Spanner TrueTime: why the commit wait is required</summary>

TrueTime returns an interval `[earliest, latest]` with uncertainty ε ≈ ±7ms. Without commit wait: T1 commits at `ts=10:00:00.007` on Node A, but Node B's clock might still read `10:00:00.000` — T2 starting on Node B could take a snapshot that excludes T1's writes even though T1 "already committed."

**The wait:** After assigning `ts_commit`, Spanner waits until `TT.now().earliest > ts_commit`. This means absolute real time has definitely passed `ts_commit` on all nodes. Any T2 starting after this point on any node gets `ts_start >= ts_commit + ε` — T1's writes are always visible.

**Average latency cost:** Assigning `ts_commit` at midpoint of `TT.now()` interval and waiting for `TT.now().earliest` to surpass it: average wait ≈ **ε = 7ms**.

**Staff-level phrasing:** "Without commit wait, a transaction on a lagging clock could start with a snapshot that excludes a just-committed write; waiting until `TT.now().earliest > ts_commit` ensures the commit timestamp is definitively in the past on every clock; cost = ~7ms per commit (one uncertainty window)."

</details>

<details>
<summary>Exercise 3 — WAL LSNs cannot be global ordering keys</summary>

PostgreSQL WAL LSNs are monotonic offsets within a single node's WAL file. Node A's LSN=5000 and Node B's LSN=5000 are independent counters — they were incremented without knowledge of each other and carry no cross-node ordering semantics.

A higher LSN on Node A does not mean "after" any LSN on Node B in real time. The two LSN sequences are completely disjoint namespaces.

**What must be layered:**
- **Lamport timestamps:** Each event carries `ts = max(local, received) + 1`. Provides deterministic total order; not real-time consistent (logical order may differ from wall-clock order).
- **Vector clocks:** Track causal dependencies per node. Detect concurrency but don't provide a total order for concurrent events.
- **TrueTime:** GPS/atomic clock bounds real-time to ±7ms. Provides real-time consistent total order at the cost of commit wait latency.

For a global ledger: **vector clocks** for causality detection + Lamport timestamp as tie-breaker for total ordering is the standard approach.

**Staff-level phrasing:** "WAL LSNs are node-local byte offsets — cross-node comparison is meaningless; layer Lamport timestamps (logical total order) or vector clocks (causal tracking) over the per-node WAL stream; TrueTime adds real-time consistency for systems willing to pay the 7ms commit-wait cost."

</details>

<details>
<summary>Exercise 4 — Discussion: VectorClock + LedgerConflictResolver design</summary>

`isConcurrent` = neither `A.happensBefore(B)` nor `B.happensBefore(A)`. The `happensBefore` check iterates all node dimensions: A ≤ B in all dimensions with at least one strictly less.

The `LedgerConflictResolver` tie-breaker (higher amount first) is a deterministic business rule ensuring all nodes resolve the same conflict identically. Without a total order tie-breaker, nodes could apply the same two concurrent transactions in different orders, producing permanently diverged balances.

**Critical constraint:** The "losing" transaction must still be applied (if valid) — conflict resolution determines ORDER, not DISCARD. A concurrent withdraw that is ordered second must still execute if the balance allows after the higher-value transaction.

**Staff-level phrasing:** "`isConcurrent` = mutual non-dominance in vector clock comparison; conflict resolver must be deterministic and total (higher-amount then UUID secondary key); the losing transaction is ORDERED SECOND, not discarded — both must eventually apply for correctness."

</details>

<details>
<summary>Exercise 5 — Cryptographic hash audit chain with Debezium CDC</summary>

**Chain design:** Each `LedgerEntry` record stores: `entry_id`, `payload`, `prev_entry_id`, `chain_hash = SHA-256(prev_chain_hash || entry_id || payload)`. Any modification to any entry invalidates all subsequent chain hashes.

**Where hash is computed:** In a dedicated Debezium CDC consumer — NOT in the application service (can be bypassed by direct DB writes). The CDC consumer reads WAL events from the `ledger` topic, retrieves the last `chain_hash` from an append-only `audit_chain` table, computes the new hash, and appends `(entry_id, payload, prev_hash, chain_hash)` — write-once, no updates.

**24-hour partition reconnect:** Debezium tracks offset (LSN) in Kafka. On reconnect, it replays from the last committed offset. The CDC consumer recomputes the chain for all replayed events in LSN order. Any tampered record during the partition shows a hash mismatch.

**Additional safeguard:** Revoke `DELETE` privilege on `ledger_entries` from all application DB roles. Only `audit_admin` (with MFA + approval workflow) can delete, and the CDC audit permanently captures the `DELETE` event in the chain regardless.

**Staff-level phrasing:** "Compute the hash chain in the Debezium CDC consumer (not application code), appending to a write-once `audit_chain` table; partition recovery replays from last committed LSN, recomputing the chain; revoke `DELETE` from application roles so no direct mutation can bypass the audit trail."
