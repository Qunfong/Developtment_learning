...
<details>
<summary>Exercise 1 — Write properties, not examples</summary>

**Four HashMap properties:**

| # | Property statement | Category |
|---|-------------------|----------|
| 1 | After `put(k, v)`, calling `get(k)` returns `v` | **Roundtrip** |
| 2 | After `put(k, v1)` then `put(k, v2)`, `size()` is the same as after the first put | **Invariant** (key uniqueness) |
| 3 | After `remove(k)`, `containsKey(k)` returns `false` | **Invariant** |
| 4 | `putAll(A)` followed by `putAll(B)` produces the same result as `putAll(B)` followed by `putAll(A)` when A and B have disjoint key sets | **Commutativity** |

**Edge case property testing finds:** **null keys and null values.** A hand-written example suite tests `put("name", "alice")` and similar string inputs. A property generator will include `null` in the generated key set — exposing any NPE in a custom `equals()`/`hashCode()` implementation or `Comparator` that doesn't handle null. The specific case of a `TreeMap` with a natural-order comparator receiving a null key throws `NullPointerException` — this is invisible to example tests that only use non-null strings.

**Staff-level phrasing:** "Properties document the invariants that make a data structure trustworthy under any input — example tests document behavior for specific inputs you anticipated."
</details>

<details>
<summary>Exercise 2 — Sort algorithm properties</summary>

```java
class StableSortTest {

    private List<Integer> stableSort(List<Integer> input) {
        return input.stream()
    ...
    @Property
    void outputSizeEqualsInputSize(@ForAll List<@Nullable Integer> input) {
        assertThat(stableSort(input)).hasSize(input.size());
    }

    @Property
    void outputIsOrdered(@ForAll List<@Nullable Integer> input) {
        List<Integer> sorted = stableSort(input);
    ...
    @Property
    void allElementsPreserved(@ForAll List<@Nullable Integer> input) {
        assertThat(stableSort(input)).containsExactlyInAnyOrderElementsOf(input);
    }

    @Property
    void sortIsIdempotent(@ForAll List<@Nullable Integer> input) {
        List<Integer> once = stableSort(input);
        List<Integer> twice = stableSort(once);
        assertThat(twice).isEqualTo(once);
    }

    @Property
    void nullsAppearAtEnd(@ForAll List<@Nullable Integer> input) {
        List<Integer> sorted = stableSort(input);
        boolean seenNull = false;
        for (Integer el : sorted) {
            if (el == null) seenNull = true;
            if (seenNull) assertThat(el).isNull();
        }
    }
}
```

**Why this works:** `@Nullable` on the `@ForAll` type annotation instructs jqwik to include `null` as a generated value. The `seenNull` flag in the nulls-at-end property enforces that once a null is encountered, every subsequent element must also be null — this is a stronger assertion than "nulls are present somewhere at the end."

**Common mistake:** Writing the idempotency property as `assertThat(stableSort(input)).isEqualTo(input)` — this tests that the list is already sorted, not that sorting twice equals sorting once.
</details>

<details>
<summary>Exercise 3 — Find the bug with shrinking</summary>

**The bug:** When `maxLength < 3`, `text.substring(0, maxLength - 3)` computes a negative end index → `StringIndexOutOfBoundsException`.

**Property that finds it:**
```java
@Property
void abbreviate_resultFitsInMaxLength(
        @ForAll @StringLength(min = 0, max = 50) String text,
        @ForAll @IntRange(min = 1, max = 50) int maxLength) {

    String result = new Abbreviator().abbreviate(text, maxLength);
    assertThat(result.length())
        .as("result for text='%s', maxLength=%d must fit within maxLength", text, maxLength)
...
**jqwik shrunk output:**
```
PropertyFailureException: Property [abbreviate_resultFitsInMaxLength] falsified
  ...
  java.lang.StringIndexOutOfBoundsException: Range [0, -2) out of bounds for length 1
      at String.substring(...)
      at Abbreviator.abbreviate(Abbreviator.java:4)
```

**Bug analysis:** `maxLength - 3` is `-2` when `maxLength = 1`. The original code assumes `maxLength >= 4` (enough room for `"..."`) but has no guard for small values.

**Fix:**
```java
public String abbreviate(String text, int maxLength) {
    if (text.length() <= maxLength) return text;
    if (maxLength <= 3) return text.substring(0, maxLength);  // guard: no room for "..."
    return text.substring(0, maxLength - 3) + "...";
}
```

**Common mistake:** Writing `@IntRange(min = 4, ...)` in the property to avoid the crash. This hides the bug instead of exposing it — the guard in the implementation must handle small maxLength values, not the test.
</details>

<details>
<summary>Exercise 4 — Custom Arbitrary</summary>
...
class EmailPropertyTest {
    ...
    @Provide
    Arbitrary<EmailAddress> validEmails() {
        Arbitrary<String> localPart = Arbitraries.strings()
            .withCharRange('a', 'z')
            .ofMinLength(1)
            .ofMaxLength(10);  // keep short for test speed
        Arbitrary<String> domainLabel = Arbitraries.strings()
            .withCharRange('a', 'z')
            .ofMinLength(1)
            .ofMaxLength(15);
        Arbitrary<String> tld = Arbitraries.of("com", "org", "net", "io");

        return Combinators.combine(localPart, domainLabel, tld)
            .as((local, domain, t) -> new EmailAddress(local + "@" + domain + "." + t));
    }

    @Property
    void emailRoundtrip(@ForAll("validEmails") EmailAddress email) {
        // Parser extracts local part and domain; Formatter reassembles them
        ParsedEmail parsed = EmailParser.parse(email.value());
        String reformatted = EmailFormatter.format(parsed);
        assertThat(reformatted).isEqualTo(email.value());
    }

    @Property
    void generatedEmailsAreStructurallyValid(@ForAll("validEmails") EmailAddress email) {
        String[] parts = email.value().split("@", 2);
        assertThat(parts).hasSize(2);
        assertThat(parts[0].length()).isBetween(1, 64);
        assertThat(parts[1]).contains(".");
        assertThat(parts[1].length()).isLessThanOrEqualTo(255);
    }
}
```

**Why this works:** `Combinators.combine(...).as(...)` generates correlated values — the three separate arbitraries are combined into a single valid email in one generation step. This is the correct pattern for records with multiple interdependent fields, versus generating each field independently and rejecting invalid combinations with `Assume.that()` (which wastes tries).

**Common mistake:** Using `Arbitraries.strings().filter(s -> s.matches(".*@.*"))` — the filter rejection rate is extremely high (almost no random string contains `@`), so jqwik exhausts its tries and the property degenerates into a no-op.
</details>

<details>
<summary>Exercise 5 — When to apply each property category</summary>

| System | Property (in English) | Category |
|--------|-----------------------|----------|
| a) REST rate limiter | Calling `tryAcquire(key)` twice with the same key within the same time window returns the same decision as the first call — no state is double-consumed | **Idempotency** |
| b) JSON serializer | For any serializable object `o`, `deserialize(serialize(o))` equals `o` | **Roundtrip** |
| c) Set union | `union(A, B)` equals `union(B, A)` for any two sets A and B | **Commutativity** |
| d) Password hashing | For any password `p`, `verify(hash(p), p)` always returns `true` | **Invariant** (correctness guarantee) |
| e) Pagination helper | For any `totalItems` and `pageSize > 0`, `totalPages * pageSize >= totalItems` — no item is lost across pages | **Invariant** |

**Deeper note on (d):** The collision resistance property — `hash(p1) != hash(p2)` for any two distinct passwords — is a security property that standard property testing *cannot* prove (it would require trying all possible inputs). The verifiable property is the affirmative roundtrip: hashing followed by verification always succeeds for the same password.

**Staff-level phrasing:** "Choosing the right property category is choosing the right contract: roundtrip for serialization, commutativity for order-independent operations, idempotency for safe-to-retry operations, invariants for correctness conditions that must hold universally."
</details>
