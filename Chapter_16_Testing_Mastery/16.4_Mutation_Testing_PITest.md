...
<details>
<summary>Exercise 1 — Identify the surviving mutants</summary>

```java
public boolean isValidPassword(String password) {
    return password != null
        && password.length() >= 8   // CONDITIONALS_BOUNDARY: >= → >
        && password.length() <= 72; // CONDITIONALS_BOUNDARY: <= → <
}
```

**All CONDITIONALS_BOUNDARY mutants:**

| Mutant | Original | Mutation | Input that kills it |
|--------|----------|----------|---------------------|
| M1 | `>= 8` | `> 8` | `password = "12345678"` (length 8) — must return `true`, mutant returns `false` |
| M2 | `<= 72` | `< 72` | `password = "a".repeat(72)` (length 72) — must return `true`, mutant returns `false` |

**Why `length = 7` and `length = 73` are insufficient alone:**

A test with `length = 7` verifies that an invalid password is rejected. The mutation `>= 8` → `> 8` still rejects length 7 — the mutant survives because both the original and the mutant return `false` for length 7. The mutation *changes* behavior only at the exact boundary: length 8 should pass (`>= 8` → true) but the mutant rejects it (`> 8` → false). Only a test asserting `isValidPassword("12345678") == true` kills M1.

Similarly for M2: length 73 is rejected by both original (`73 <= 72` is false) and mutant (`73 < 72` is false). The mutation `<= 72` → `< 72` diverges only at length 72 — a test asserting `isValidPassword("a".repeat(72)) == true` is required.

**Staff-level phrasing:** "Off-by-one mutations are invisible to tests that avoid the exact boundary — the only way to kill a boundary mutant is to test the value that sits exactly on the line between valid and invalid."
</details>

<details>
<summary>Exercise 2 — Kill the surviving mutants</summary>
...
**Typical LoanEligibilityChecker surviving mutants and kills:**

```java
// Original (typical weak test only tests score = 500 and score = 800)
public boolean isEligible(int creditScore, double debtRatio) {
    return creditScore >= 700 && debtRatio < 0.4;
}

// Surviving mutants before improvements:
// M1: >= 700 → > 700   (test only used score=800, not score=700)
// M2: < 0.4  → <= 0.4  (test only used ratio=0.3, not ratio=0.4)

// Tests to add that kill each survivor:
@Test void exactCreditScoreBoundary_isEligible() {
    assertThat(checker.isEligible(700, 0.3)).isTrue();   // kills M1
}

@Test void exactDebtRatioBoundary_isNotEligible() {
    assertThat(checker.isEligible(750, 0.4)).isFalse();  // kills M2
}
```

**Typical result:** mutation score before: ~58%, after adding 2 boundary tests: ~91%. The HTML report (`target/pit-reports/*/index.html`) shows each mutant in green (killed) or red (survived) with the exact source line.

**Common mistake:** Adding `score = 699` and `score = 701` instead of `score = 700`. These test around the boundary but not on it — the `>=`→`>` mutation still survives.
</details>

<details>
<summary>Exercise 3 — Mutation-resistant test for RangeClamp</summary>
...
class RangeClampTest {
    private final RangeClamp clamp = new RangeClamp();

    // Kills CONDITIONALS_BOUNDARY: < → <= at the min boundary
    @Test void exactlyAtMin_returnsMin() {
        assertEquals(5, clamp.clamp(5, 5, 10));
    }

    // Kills CONDITIONALS_BOUNDARY: > → >= at the max boundary
    @Test void exactlyAtMax_returnsMax() {
        assertEquals(10, clamp.clamp(10, 5, 10));
    }

    // Kills NEGATE_CONDITIONALS on value < min
    @Test void belowMin_returnsMin() {
        assertEquals(5, clamp.clamp(3, 5, 10));
    }

    // Kills NEGATE_CONDITIONALS on value > max
    @Test void aboveMax_returnsMax() {
        assertEquals(10, clamp.clamp(12, 5, 10));
    }

    // Kills CONDITIONALS_BOUNDARY at min+1 (verifies value passes through unchanged)
    @Test void withinRange_returnsValue() {
        assertEquals(7, clamp.clamp(7, 5, 10));
    }

    // Kills any RETURN_VALUE mutation that would always return min
    @Test void differentMinAndMaxValuesAreDistinguished() {
        assertEquals(3, clamp.clamp(1, 3, 8));
        assertEquals(8, clamp.clamp(9, 3, 8));
    }
}
```

**Mutation score analysis:** With these 6 tests, PITest kills:
- 2 CONDITIONALS_BOUNDARY mutants (exact min, exact max)
- 2 NEGATE_CONDITIONALS mutants (below min, above max)
- 2 RETURN_VALUE mutants (within range, different values)

Achievable mutation score: **~95%**. The only survivors are typically `void` method mutations (none in this class) and arithmetic mutations on `return value` which have no variants for a simple identity return.
</details>

<details>
<summary>Exercise 4 — Selective mutation configuration</summary>

```xml
<!-- pom.xml — PR pipeline execution (incremental) -->
<plugin>
  <groupId>org.pitest</groupId>
  <artifactId>pitest-maven</artifactId>
  <version>1.15.3</version>
  <executions>
    ...
    <!-- Billing package — high threshold -->
    <execution>
      <id>mutation-billing</id>
      ...
      <configuration>
        ...
        <historyInputFile>${project.build.directory}/pit-history/billing.bin</historyInputFile>
        <historyOutputFile>${project.build.directory}/pit-history/billing.bin</historyOutputFile>
      </configuration>
    </execution>

    <!-- Pricing package — lower threshold -->
    <execution>
      <id>mutation-pricing</id>
      <goals><goal>mutationCoverage</goal></goals>
      <configuration>
        <targetClasses><param>com.example.pricing.*</param></targetClasses>
        <targetTests><param>com.example.pricing.*Test</param></targetTests>
        <mutationThreshold>75</mutationThreshold>
        <historyInputFile>${project.build.directory}/pit-history/pricing.bin</historyInputFile>
        <historyOutputFile>${project.build.directory}/pit-history/pricing.bin</historyOutputFile>
      </configuration>
    </execution>
...
```yaml
# PR pipeline (incremental, < 5 min)
- name: Mutation Tests (PR)
  run: mvn pitest:mutationCoverage -Dpitest.withHistory=true
  # incremental history means only mutants in changed classes are re-evaluated

# Nightly pipeline (full, no history restriction)
- name: Mutation Tests (Nightly Full)
  run: mvn pitest:mutationCoverage
  # no -Dpitest.withHistory — runs all mutants for complete picture
  schedule: '0 2 * * *'  # 2am daily
```

**Note on per-package thresholds:** PITest does not support a single `<mutationThreshold>` that differs per package in one execution block. The solution is separate `<execution>` blocks — each targeting one package with its own threshold. The build fails if either execution falls below its threshold.
</details>

<details>
<summary>Exercise 5 — The cost argument</summary>

**5-point business case for PITest in CI:**

**1. Off-by-one bugs are the most common class of silent financial bug — and they're invisible to line coverage.**
The incident in this module: `processingFee` applied when `amount == feeThreshold` when it should have been excluded. 100% line coverage passed. The mutation `>=` → `>` on the threshold check was not killed. Mutation testing catches this class of bug by construction.

**2. 52% mutation score means ~48% of detectable bug categories reach production undetected.**
This is not a theoretical risk — it is the current state of most Java codebases with high line coverage and weak assertions. Each survived mutant maps to a real bug category: boundary errors, negated conditions, removed side effects, wrong return values.

**3. The 15-minute cost applies only if you run it wrong.**
Incremental PITest (`withHistory`, targeting only changed packages) runs in under 3 minutes on a typical PR touching 2-3 files. The 15-minute figure is a full-suite cold run. The nightly job can run the full suite while the PR gate uses incremental — total engineering overhead is near zero.

**4. The alternative cost: one boundary bug in billing = one week of engineering.**
The real incident took 3 days to diagnose and 1 day to fix (4 engineering days × 2 engineers × $750/day loaded cost = **$6,000**). PITest would have caught it in the first PR. Payback period at 1 incident per 6 months: 15 minutes of CI time per day, 180 days = 45 hours of CI time, to prevent a $6,000 incident.

**5. Mutation score is the only metric that predicts test suite quality under adversarial conditions.**
Line coverage measures what you ran. Mutation score measures what your tests can *detect*. A payment gateway with 100% line coverage and 52% mutation score has the same defect exposure as a gateway with 50% line coverage — the number just looks better. Mutation score is the honest metric for "are our tests good?"

**Staff-level phrasing:** "Mutation testing is not a quality luxury — it is the only automated way to measure whether your test suite would detect the bugs that actually cause production incidents, and the incremental mode means the 'it slows the build' objection was never really about the tool."
</details>
