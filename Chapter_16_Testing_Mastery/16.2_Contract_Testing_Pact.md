...
<details>
<summary>Exercise 1 — Consumer vs Provider ownership</summary>

Consumer-driven contracts produce better outcomes because they capture what each consumer **actually uses**, not what the provider assumes consumers need. A provider-driven contract documents the full API surface — the provider must conservatively maintain every field to avoid breaking unknown consumers.

With 5 consumers, a provider-driven approach requires the provider to ask each team "do you use `legacyId`?" — manually, asynchronously, across team boundaries. Consumer-driven contracts make this automatic: the pact file each consumer publishes either references `legacyId` or it doesn't. The provider can query the Pact Broker and immediately see which of the 5 consumers use the field. If only 2 reference it, the other 3 have implicitly voted "we don't need it" — the provider can remove it knowing exactly who is affected.

Provider-driven contracts also create perverse incentives: providers add fields proactively "in case someone needs it," inflating the schema and coupling consumers to undocumented structure they never requested.

**Staff-level phrasing:** "Consumer-driven contracts are API governance by contract — each consumer publishes a legally binding statement of what it uses, and the provider can only remove fields that zero consumers are claiming."
</details>

<details>
<summary>Exercise 2 — Add a new consumer interaction</summary>

```java
// Consumer test — add to the existing PactConsumerTest class
@Pact(consumer = "UserServiceConsumer")
public RequestResponsePact deleteUserPact(PactDslWithProvider builder) {
    return builder
        .given("user with id 5 exists")
        .uponReceiving("DELETE request for user 5")
        .method("DELETE")
        .path("/users/5")
        .willRespondWith()
        .status(204)
...
void deleteUser_returns204NoContent(MockServer mockServer) {
    UserApiClient client = new UserApiClient(mockServer.getUrl());
    boolean deleted = client.deleteUser(5L);
    assertThat(deleted).isTrue();
}

// Provider state setup — add to the existing ProviderStateChangeController or @State class
@State("user with id 5 exists")
public void setupUserWithId5() {
...
**Why this works:** The `given("user with id 5 exists")` string must exactly match the `@State` annotation value on the provider side — Pact uses string equality to route state setup. The `204 No Content` response has no body, so no body DSL is needed.

**Common mistake:** Omitting the provider state and relying on whatever is in the DB. If the test runs in an environment where id 5 doesn't exist, the DELETE returns 404 and the verification fails — state setup makes the test deterministic.
</details>

<details>
<summary>Exercise 3 — Simulate and detect a breaking change</summary>

**Introducing the break:** In the `TransactionResponse` record, rename `transaction_id` → `transactionId` without `@JsonProperty`:

```java
// Breaking change — field name changes in JSON output
public record TransactionResponse(String transactionId, BigDecimal amount) {}
```

**Provider verification failure output:**
```
1 interaction failed

Description: GET /transactions/42 returns transaction details

Failures:

  1) $.transaction_id
       Expected 'TXID-42' (String) but was missing
       Actual body received: {"transactionId":"TXID-42","amount":100.00}

       Expected: {"transaction_id":"TXID-42", ...}
       Actual:   {"transactionId":"TXID-42", ...}
```

**Restoring backward compatibility with `@JsonProperty`:**
```java
public record TransactionResponse(
    @JsonProperty("transaction_id") String transactionId,  // Java uses camelCase internally
    BigDecimal amount
) {}
```

The JSON output reverts to `transaction_id` — provider verification passes. The Java record field is `transactionId` (camelCase), but the API contract is `transaction_id` (snake_case). This is a clean separation of internal naming from external contract.
</details>

<details>
<summary>Exercise 4 — Pact Broker deployment gate</summary>

**Consumer pipeline:**
```
[Build] → [Unit Tests] → [Pact Consumer Tests] → [Publish Pact to Broker] → [can-i-deploy?] → [Deploy to Staging]
```
- Pact is published **after consumer tests pass**, before any deploy. The published version tag matches the consumer's git commit SHA.

**Provider pipeline:**
```
[Build] → [Unit Tests] → [Integration Tests] → [Pact Provider Verification] → [can-i-deploy?] → [Deploy to Staging]
```
- Provider verification runs **after the provider's own tests pass**, pulling pacts from the Broker for all registered consumers.
...
```
Runs in **both** pipelines before the respective deploy step. It checks: "has this version been verified against all contracts it depends on?"

**If provider verification fails:**
- The **provider's pipeline fails** — the failing step is `Pact Provider Verification`, which is a gate before deploy
- The consumer's pipeline is **not immediately affected** (it already published and can-i-deploy checks the broker state)
- The consumer is **blocked from deploying** if `can-i-deploy` checks that the provider version it expects hasn't passed verification — the failure propagates through the shared verification state in the Broker

**Staff-level phrasing:** "Pact Broker turns contract verification into a shared deployment gate — neither side can ship a version that breaks a verified contract, regardless of which team introduced the incompatibility."
</details>

<details>
<summary>Exercise 5 — Pact limitations</summary>

**a) Event-based interactions (Kafka):**
Pact is designed for synchronous request-response (HTTP). For Kafka topics, use:
- **Schema Registry with `BACKWARD_TRANSITIVE` compatibility**: the registry enforces that every new schema version can be read by all older consumers — this is contract enforcement at the schema level, without Pact
- **Spring Cloud Contract** with its messaging DSL, which supports triggering a message and asserting on the consumed payload structure
- Pact 4.x has a `MessagePact` API for async interactions, but it lacks the ecosystem maturity of the HTTP version — treat it as experimental for production use

**b) Complex server-side filtering (query parameters):**
Pact's query parameter matching treats parameters as independent key-value pairs with simple type/regex matchers. A consumer test for `?status=active&sort=createdAt&direction=desc&page=2&pageSize=50` with interdependent validation (e.g., `direction` only valid when `sort` is provided) cannot be expressed as a Pact interaction without an explosion of interaction combinations.
Use instead:
- **Spring Cloud Contract with Groovy DSL**: supports request templates, conditional matching, and response body depending on query parameters
- **End-to-end integration tests** in a pre-production environment for complex filtering paths — these are genuinely cross-cutting behaviors that require a real server to evaluate

**Staff-level phrasing:** "Pact solves the structural contract problem for HTTP request/response — reach for Schema Registry when the contract lives in a message schema, and Spring Cloud Contract when the interaction logic is too stateful for Pact's request-response model."
</details>
